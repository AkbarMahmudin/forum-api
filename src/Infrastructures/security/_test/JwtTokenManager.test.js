...
describe('JwtTokenManager', () => {
  ...
  describe('authorize function', () => {
    it('should authorize correctly', async () => {
      ...
      // Action
      const { username: expectedUsername } = await jwtTokenManager.authorize(headers);

      // Action & Assert
      expect(expectedUsername).toEqual('dicoding');
    });
    
    it('should throw AuthorizeError when headers empty', async () => {
      // Arrange
      const jwtTokenManager = new JwtTokenManager(Jwt.token);

      // Action & Assert
      await expect(jwtTokenManager.authorize('')).rejects.toThrow(AuthenticationError);
      await expect(jwtTokenManager.authorize('Bearer ')).rejects.toThrow(AuthenticationError);
    });
  });
});
